...
"""
AQUA FDB catalog generator class.
"""
...
def get_nested(cfg, key):
    """Helper function to get nested values from a dictionary using a tuple of keys."""
...
class AquaFDBGenerator:
    ...
    def __init__(
        self, data_portfolio: str, config_path: str, configcontext: ConfigContext | None = None, loglevel: str = "INFO"
    ):
        """
        ...
        Args:
            ...
            configcontext (ConfigContext, optional): An instance of ConfigContext for configuration management.
                                                     Defaults to None.
        ...
        configcontext = configcontext or ConfigContext()
        ...
        missing = [k if isinstance(k, str) else ".".join(k) for k in required_keys if not get_nested(self.config, k)]
    ...
    def get_time(frequency, levtype):
        ...
        freq2time = {
            ...
            "6-hourly": {"time": "0000", "chunks": "6h", "savefreq": "h"},
            "daily": {"time": "0000", "chunks": "D", "savefreq": "D"},
            "monthly": {"time": None, "chunks": "MS", "savefreq": "MS"},
        }
    ...
    def create_catalog_entry(self, all_content):
        ...
        """
        output_dir = os.path.join(self.catalog_dir_path, "catalogs", self.config["catalog_dir"], "catalog", self.model)
        ...
        with SafeFileLock(main_yaml_path + ".lock", loglevel=self.loglevel):
            ...
            resolution_map = {"production": "HR", "develop": "SR", "lowres": "LR", "intermediate": "MR"}
            ...
            main_yaml["sources"][self.config["exp"]] = {
                ...
                "args": {"path": f"{{{{CATALOG_DIR}}}}/{self.config['exp']}.yaml"},
            }
        ...
        with SafeFileLock(catalog_yaml_path + ".lock", loglevel=self.loglevel):
            ...
            if self.model not in catalog_yaml.get("sources", {}):
                ...
                catalog_yaml["sources"][self.model] = {
                    ...
                    "driver": "yaml_file_cat",
                    "args": {"path": f"{{{{CATALOG_DIR}}}}/catalog/{self.model}/main.yaml"},
                }
    ...
    def generate_catalog(self):
        ...
        for profile in self.dp[model_key]:  # Filter out omitted resolutions, if any
            current_resolutions = [
                res
