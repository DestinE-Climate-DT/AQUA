...
def analysis_parser(parser=None):
    ...
    parser.add_argument(
        "--local_clusters", action="store_true", help="Use separate local clusters instead of single global one"
    )
    ...
    if "DASK_DISTRIBUTED__COMM__TIMEOUTS__CONNECT" not in os.environ:
        ...
        if connect_timeout:
            os.environ["DASK_DISTRIBUTED__COMM__TIMEOUTS__CONNECT"] = (
                f"{connect_timeout}s"  # increase timeout (certainly needed on lumi, possibly good anyway)
            )
