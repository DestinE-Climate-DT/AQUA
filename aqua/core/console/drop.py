...
def drop_parser(parser=None):
    ...
    parser.add_argument("-f", "--fix", action="store_true", help="fixer on existing data")
    ...
    parser.add_argument(
        "--monitoring", action="store_true", help="enable the dask performance monitoring. Will run a single chunk"
    )
    parser.add_argument(
        "--only-catalog", action="store_true", help="To not run DROP but simply create the catalog entries for netcdf and zarr"
    )
    parser.add_argument(
        "--catalog", type=str, help="catalog to be processed. Use with coherence with --model, -exp and --source"
    )
    parser.add_argument("-m", "--model", type=str, help="model to be processed. Use with coherence with --exp and --source")
    parser.add_argument(
        "-e", "--exp", type=str, help="experiment to be processed. Use with coherence with --source and --model"
    )
    parser.add_argument("-s", "--source", type=str, help="source to be processed. Use with coherence with --exp and --var")
    parser.add_argument("-v", "--var", type=str, help="var to be processed. Use with coherence with --source")
    parser.add_argument("--rebuild", action="store_true", help="Rebuild Reader areas and weights")
    parser.add_argument(
        "--realization", type=str, help="realization to be processed. Use with coherence with --model, --exp and --source"
    )
    parser.add_argument("--stat", type=str, help="statistic to be computed. Can be one of ['min', 'max', 'mean', 'std'].")
    parser.add_argument("--frequency", type=str, help="Frequency of the DROP output. Can be anything in the AQUA frequency.")
    parser.add_argument(
        "--resolution", type=str, help="Resolution of the DROP output. Can be anything in the AQUA resolution."
    )
    parser.add_argument("--startdate", type=str, help="Start date to subset the data. Format YYYY-MM-DD")
    parser.add_argument("--enddate", type=str, help="End date to subset the data. Format YYYY-MM-DD")
    parser.add_argument(
        "--engine", type=str, help="Engine to be used for GSV retrieval: 'polytope' or 'fdb'. Defaults to 'fdb'."
    )
    parser.add_argument("--zarr", action="store_true", help="Create zarr")
    parser.add_argument("--verify-zarr", action="store_true", help="Verify the created zarr")
