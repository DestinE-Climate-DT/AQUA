...
def boxplot(
    ...
    for ds, model in zip(fldmeans, model_names):
        for var in variables:
            ...
            for val in data:
                records.append({"Variables": labels[var], "Values": val, "Models": model})
    ...
    # Plot the boxplots
    sns.boxplot(data=df, x="Variables", y="Values", hue="Models", order=order, hue_order=hue_order, width=0.8, ax=ax)
    ...
    if add_mean_line:
        ...
        offsets = np.linspace(-total_box_width / 2 + single_box_width / 2, total_box_width / 2 - single_box_width / 2, n_hues)
        ...
        for i, var in enumerate(order):
            for j, model in enumerate(hue_order):
                ...
                ax.hlines(mean_val, x_left, x_right, colors=[darker], linestyles="--", linewidth=2.5, zorder=5)
