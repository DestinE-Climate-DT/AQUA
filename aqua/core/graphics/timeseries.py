...
def plot_timeseries(
    monthly_data: list[xr.DataArray] | xr.DataArray = None,
    ...
    if data_labels is not None:
        ...
        if suffix:
            data_labels = {
                "monthly": [f"{label} monthly" for label in data_labels],
    ...
    if ref_monthly_data is not None:
        plot_timeseries_ref_data(
            ax=ax, ref_data=ref_monthly_data, std_data=std_monthly_data, ref_label=ref_label["monthly"], lw=0.8, kind="monthly"
        )

    if ref_annual_data is not None:
        plot_timeseries_ref_data(
            ax=ax, ref_data=ref_annual_data, std_data=std_annual_data, ref_label=ref_label["annual"], lw=0.8, kind="annual"
        )

    if ens_monthly_data is not None:
        plot_timeseries_ensemble(
            ax=ax,
            data=ens_monthly_data,
    ...
    if ens_annual_data is not None:
        plot_timeseries_ensemble(
            ax=ax, data=ens_annual_data, std_data=std_ens_annual_data, data_label=ens_label["annual"], lw=0.8, kind="annual"
        )
...
def plot_seasonalcycle(
    data: list[xr.DataArray] | xr.DataArray,
    ...
    if ref_data is not None:
        try:
            ...
            if std_data is not None:
                ...
                std_data.compute()
                ax.fill_between(
                    ref_data.month, ref_data - 2.0 * std_data, ref_data + 2.0 * std_data, facecolor="grey", alpha=0.5
                )
