...
class GSVSource(base.DataSource):
    ...
    def to_dask(self):
        ...
        for var in self._ds.data_vars:
            ...
            if updated_var != var:
                self.logger.warning(
                    "Variable shortname %s has been interpreted with another eccodes. Current eccodes %s will read paramid %s as %s",
                    var,
                    eccodes.__version__,
                    original_paramid,
                    updated_var,
            ...
            if not self.chunking_vertical:
                ...
                for j in range(self.nlevelchunks):
                    dalistlev = [
                        self.get_part_delayed(i * self.nlevelchunks + j, original_paramid, shape, dtype)
