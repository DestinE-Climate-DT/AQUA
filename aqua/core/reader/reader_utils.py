...
def check_catalog_source(cat, model, exp, source, name="dictionary"):
    ...
    if model not in cat:
        avail = list(cat.keys())
        raise KeyError(f"Model {model} not found in {name}. Please choose between available models: {avail}")
    if exp not in cat[model]:
        avail = list(cat[model].keys())
        raise KeyError(
            f"Experiment {exp} not found in {name} for model {model}. Please choose between available exps: {avail}"
        )
    ...
    if source:
        if source not in cat[model][exp]:
            if "default" not in cat[model][exp]:
                ...
                raise KeyError(
                    f"Source {source} of experiment {exp} "
