...
class CFTimeHandler:
    ...
    def average_datetimes(self, dt1, dt2, reference="days since 1850-01-01"):
        ...
        if isinstance(dt1, cftime.datetime):
            ...
            return cftime.num2date(
                avg_num, units=reference, calendar=dt1.calendar)
    ...
    def add_offset(self, dt, offset, reference="days since 1850-01-01"):
        ...
        if isinstance(dt, cftime.datetime):
            ...
            days_since = (dt - base_date).days + \
                (dt - base_date).seconds / 86400.0
            ...
            return cftime.num2date(
                new_days, units=reference, calendar=calendar)
    ...
    def center_time_axis(self, avg_data: xr.Dataset,
                         resample_freq: str) -> xr.Dataset:
        """
            ...
            for t in time_values:
                ...
                if 'MS' in resample_freq:
                    ...
                    if t.month == 12:
                            ...
                            t.year + 1, 1, 1, calendar=calendar)
                    else:
                        end = cftime.datetime(
    ...
    def check_chunk_completeness(
            self, xdataset, resample_frequency='1D', loglevel='WARNING'):
        """
        ...
        if len(chunks) == 0:
            raise ValueError(
                f'No chunks! Cannot compute average on {resample_frequency} period')

        check_completeness = []

        for chunk_start in chunks:
            # Calculate expected end date
            end_date = self._find_end_date_cftime(
                chunk_start, resample_frequency)
            logger.debug(
                'Processing chunk from %s to %s',
                chunk_start,
                end_date)
        ...
        return xr.DataArray(check_completeness, dims=(
            'time',), coords={'time': taxis.time})

    def check_seasonal_chunk_completeness(
            self, xdataset, resample_frequency='QS-DEC', loglevel='WARNING'):
        """Not yet implemented for CFTime."""
        raise NotImplementedError(
            "Seasonal chunk completeness not yet implemented for CFTime")
    ...
    def _find_end_date_cftime(self, start_date, offset_str):
        ...
        if 'D' in offset_str:
            ...
            return cftime.datetime(
                start_date.year + years, 1, 1, calendar=calendar)
    ...
    def _generate_expected_time_series_cftime(
            self, start_date, frequency, time_period):
        """
