...
def parse_arguments(arguments):
    ...
    parser.add_argument("-w", "--workers", type=str, default="1", help="number of dask workers")
    parser.add_argument("-l", "--loglevel", type=str, default="WARNING", help="log level [default: WARNING]")
...
class Benchmarker:
    ...
    def __init__(self, model=None, exp=None, source=None, nproc=1, nrepeat=5, loglevel="WARNING"):
        """
    ...
    def benchmark_fldmean(self, tsteps=100):
        ...
        single = timeit(
            lambda: reader.retrieve(var="2t").isel(time=slice(0, tsteps)).aqua.fldmean().compute(), number=self.nrepeat
        )
        return round(single / self.nrepeat, 1)
    ...
    def benchmark_regrid(self, tsteps=50):
        ...
        single = timeit(
            lambda: reader.retrieve(var="2t").isel(time=slice(0, tsteps)).aqua.regrid().compute(), number=self.nrepeat
        )
        return round(single / self.nrepeat, 1)
...
if __name__ == "__main__":
    ...
    Bench = Benchmarker(model=model, exp=exp, source=source, nproc=int(args.workers), loglevel=args.loglevel)

    # create a list of methods and their names
    methods = [
        (Bench.benchmark_reader, "Reader initialization"),
