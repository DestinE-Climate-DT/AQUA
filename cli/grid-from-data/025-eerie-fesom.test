...
if __name__ == '__main__':
    ...
    logger = log_configure(log_level=loglevel, log_name='config-eerie-fesom')

    file = get_arg(args, 'config', 'config-eerie-fesom.yaml')
    ...
    create_folder(tgt)
    if model3d:
        grid_name = 'eerie-fesom-0.25deg-3d'
    else:
        grid_name = 'eerie-fesom-0.25deg-2d'
