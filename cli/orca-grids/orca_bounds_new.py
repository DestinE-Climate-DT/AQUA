...
class OrcaMesh(metaclass=abc.ABCMeta):
    ...
    def _set_mesh_attrs(self):
        ...
        ds_out["lat"].attrs = {"standard_name": "latitude", "units": "degrees North", "bounds": "lat_b"}

        ds_out["lon"].attrs = {"standard_name": "longitude", "units": "degrees East", "bounds": "lon_b"}
    ...
    def _geom_to_xesmf(self, meshfile):
        ...
        get_vars = ["glam" + self.stagg, "gphi" + self.stagg, "e1" + self.stagg, "e2" + self.stagg, self.stagg + "mask"]
        ...
        ds_mesh = ds_mesh.rename({"glam" + self.stagg: "lon", "gphi" + self.stagg: "lat"}).set_coords(["lat", "lon"])

        ds_mesh = ds_mesh.rename({self.stagg + "mask": "mask"})
        ds_mesh["mask"].attrs = {"standard_name": "mask", "flag_values": "0; 1", "flag_meanings": "dry; wet"}
        ...
        ds_mesh["cell_area"].encoding = {"_FillValue": -1.0e20, "dtype": "int64"}
        ...
        if self.level:
            ...
            ds_mesh = ds_mesh.drop_vars(["gdept_1d"]).assign_coords({self.VDIM: da_zaxis})
            ds_mesh[self.VDIM].attrs = {
                "standard_name": "depth",
                "positive": "down",
                "units": "m",
    ...
    def _get_bounds_coords(self, ds_mesh, grid_ctn):
        ...
        for coord in ["lon", "lat"]:
            ...
            tmp_bnds.loc[dict(x_b=slice_main_x, y_b=slice_main_y)] = ds_all_coords[crn_info["crn"]][coord].data
    ...
    def get_ds_cf(self):
        ...
        for coord in ["lat", "lon"]:
            ...
            bnds_cf.attrs = {}
            bnds_cf.loc[{self.BNDS_DIM: 0}] = bnds_xesmf.isel({"x_b": slice(0, nx), "y_b": slice(0, ny)}).data
            bnds_cf.loc[{self.BNDS_DIM: 1}] = bnds_xesmf.isel({"x_b": slice(1, nx + 1), "y_b": slice(0, ny)}).data
            bnds_cf.loc[{self.BNDS_DIM: 2}] = bnds_xesmf.isel({"x_b": slice(1, nx + 1), "y_b": slice(1, ny + 1)}).data
            bnds_cf.loc[{self.BNDS_DIM: 3}] = bnds_xesmf.isel({"x_b": slice(0, nx), "y_b": slice(1, ny + 1)}).data
        ...
        ds_out["dummy"] = xr.ones_like(ds_out["mask"], dtype=float).where(ds_out["mask"] > 0.5)
        ds_out["dummy"].encoding = {"dtype": "float32", "_FillValue": self.FILLVAL}
    ...
    def reorder_vars(self, dset):
        ...
        vvars += ["lat", "lat_bnds", "lon", "lon_bnds", "cell_area", "mask", "dummy"]
    ...
    def _get_all_coords(ds_mesh):
        ...
        all_coords = {}
        for grid in grids:
            all_coords[grid] = {"lon": ds_mesh["glam" + grid], "lat": ds_mesh["gphi" + grid]}
...
def get_args():
    ...
    parser.add_argument("--level", action=argparse.BooleanOptionalAction, help="include vertical axis")
    parser.add_argument("--zname", type=str, default="level", help="vertical axis name")
