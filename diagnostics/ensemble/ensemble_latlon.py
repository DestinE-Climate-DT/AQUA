...
class EnsembleLatLon():
    ...
    def plot(self):
        ...
        if self.dataset_mean.sizes != 0:
            ...
            levels = np.linspace(self.dataset_mean['2t'].values.min(),self.dataset_mean['2t'].values.max(),num=21)
            ...
            im = ax0.contourf(self.dataset_mean.lon,self.dataset_mean.lat,self.dataset_mean['2t'],cmap=cmap,levels=levels,extend='both')
            ax0.set_title(f'Map of {var} for Ensemble Multi-Model Mean')
            ax0.set_xlabel('Longitude')
            ax0.set_ylabel('Latitude')
            cbar = fig0.colorbar(im, ax=ax0, shrink=0.4, extend='both')
            cbar.set_label(f"{self.dataset_mean['2t'].GRIB_name} in {self.dataset_mean['2t'].units}")
        ...
        if self.dataset_std.sizes != 0:
            ...
            levels = np.linspace(self.dataset_std['2t'].values.min(),self.dataset_std['2t'].values.max(),num=21)
            ...
            im = ax1.contourf(self.dataset_std.lon,self.dataset_std.lat,self.dataset_std['2t'],cmap=cmap,levels=levels,extend='both')
            ...
            cbar.set_label(f"{self.dataset_std['2t'].GRIB_name} in {self.dataset_std['2t'].units}")
