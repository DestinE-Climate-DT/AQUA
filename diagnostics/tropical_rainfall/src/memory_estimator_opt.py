...
def expected_total_memory_usage_opt(ds_full):

    VmRSS_1 = read_VmRSS()
    
    """ TEST PART   TEST PART   TEST PART   TEST PART   TEST PART """ 

    runpy.run_path(path_name=str(pwd)+'/../notebooks/time_memory_estimation/memory_test.py')
    sys.path.append(str(pwd)+'/../notebooks/time_memory_estimation/')
    from memory_test import TEST_SIZE
    ...
    Mem_Consumed_by_Single_Object = Mem_Consumed/TEST_SIZE
...
def adaptive_data_load_opt(ds_full, Mem_Perc_Max = 0.5):
